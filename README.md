...
- 播放器的“本地音乐”按钮改为“加入本地歌单”，将本地曲目去重追加到当前列表，不中断正在播放的歌曲。
- 新增“清空当前歌单”按钮；只清空播放器列表，不删除硬盘文件、账号歌单、Cookie 或听歌统计。
- 搜索区域改为两行布局：第一行搜索与解析，第二行放置两个歌单操作按钮，减少窄屏拥挤。
- 播放、载入音频和页面从后台恢复时，自动将插件播放器的播放速率校准为正常的 `1×`。

> 如果音乐与 SillyTavern 生成提示音同时出现倍速，通常是 Android WebView 从后台恢复后的全局媒体时钟异常；完整重载酒馆页面可以重建媒体环境。本版本的速率校准只作用于插件自己的音乐播放器。
